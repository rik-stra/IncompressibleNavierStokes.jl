...
for (name, n_replicas) in zip(linreg_params_table.name, linreg_params_table.n_replicas)
    ...
    for i in 1:n_replicas
        file = @__DIR__()*"/output/online_TOpaper/$(name)/LF_online_channel_to_64_64_32_tsim10.0_repl_$(i).jld2"
        if isfile(file)
...
save(@__DIR__()*"/output/online_TOpaper/ks_dists_range_$(index_range[1])_$(index_range[2]).jld2", "ks_table", linreg_params_table)
