...
for i in 1:size(q_ref, 1)
    for j in 1:n_replicas
            ...
            model=lines!(axs[i],time_index, q_rep[i,:,j], color = (:blue, 0.2))
        
    end
    ref = lines!(axs[i], time_index, q_ref[i,:], color = :black)
    no_model = lines!(axs[i], time_index, no_sgs_data[i,:], color = (:red, 0.6))
    
end
Label(g[-1, :], text = "ANN tanh \n hist = $(ANN_parameters.hist_len)", fontsize = 20)
