...
if hist_var == :q
    inputs,outputs = create_history(hist_len, data.q_star[:,train_range[1]:train_range[2]], data.q[:,train_range[1]:train_range[2]], data.dQ[:,train_range[1]:train_range[2]]; include_predictor)
elseif hist_var == :q_star
    inputs,outputs = create_history(hist_len, data.q_star[:,train_range[1]+1:train_range[2]], data.q_star[:,train_range[1]:train_range[2]-1], data.dQ[:,train_range[1]+1:train_range[2]]; include_predictor)
elseif hist_var == :q_star_q
    inputs,outputs = create_history(hist_len, data.q_star[:,train_range[1]+1:train_range[2]], cat(data.q[:,train_range[1]+1:train_range[2]],data.q_star[:,train_range[1]:train_range[2]-1],dims = 1), data.dQ[:,train_range[1]+1:train_range[2]]; include_predictor)
end
...
## test the model
dir = @__DIR__()*"/output/LinReg13/"
model = load(dir*"LinReg.jld2")
hist_var = model["hist_var"]
include_predictor = model["include_predictor"]
#hist_var = :q
if hist_var == :q
    inputs_test,outputs_test = create_history(model["hist_len"], data.q_star[:,1:8000], data.q[:,1:8000], data.dQ[:,1:8000]; include_predictor)
elseif hist_var == :q_star
    inputs_test,outputs_test = create_history(model["hist_len"], data.q_star[:,2:8000], data.q_star[:,1:8000-1], data.dQ[:,2:8000]; include_predictor)
elseif hist_var == :q_star_q
    inputs_test,outputs_test = create_history(model["hist_len"], data.q_star[:,2:8000], cat(data.q[:,2:8000],data.q_star[:,1:8000-1],dims = 1), data.dQ[:,2:8000]; include_predictor)
end
