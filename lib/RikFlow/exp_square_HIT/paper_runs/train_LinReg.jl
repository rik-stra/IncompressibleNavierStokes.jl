...
## test the model
data_test = load(@__DIR__()*"/output/tracking/data_track_trackingnoise_std_$(tracking_noise)_Re2000.0_tsim10.0_replica1.jld2", "data_track");
...
q_test = RikFlow.scale_input(data_test.q[:,1:4000], model["scaling"].in_scaling)
q_star_test = RikFlow.scale_input(data_test.q_star[:,1:4000], model["scaling"].in_scaling)
dQ_test = RikFlow.scale_input(data_test.q[:,2:4001], model["scaling"].out_scaling)
...
plot_time_series(rand_unsc, qois, "rand_part", ref=stds_ref_data.*tracking_noise.*randn(6,4000))

# plot original time series
preds_unsc = RikFlow.scale_output(preds, model["scaling"].out_scaling)
plot_time_series(preds_unsc, qois, "preds_unsc", ref = data_test.q[:,model["hist_len"]+1:4000])
