# create database with KS distances for TO LRS simulations
using JLD2
using DataFrames
using IncompressibleNavierStokes
using RikFlow

index_range = [1,4] # range of linreg models to compute KS distances for
inputs_file = "/inputs_example.jld2"
TO_folder = @__DIR__()*"/output/TO_LRS"

linreg_params_table = DataFrame(load(TO_folder*inputs_file, "inputs"))[index_range[1]:index_range[2],:]
# load reference data
...
for (name, n_replicas) in zip(linreg_params_table.name, linreg_params_table.n_replicas)
    ...
    for i in 1:n_replicas
        file = TO_folder*"/$(name)/data_online_tsim100.0_replica$(i).jld2"
        if isfile(file)
    ...
    q_rep = [load(TO_folder*"/$(name)/data_online_dns512_les64_Re2000.0_tsim100.0_replica$(i)_rand_initial_dQ.jld2", "data_online").q for i in stable_sims]
    qs = cat(q_rep..., dims = 2)
